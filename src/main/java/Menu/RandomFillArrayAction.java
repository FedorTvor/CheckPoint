...
import Class.*;
import Fill.BookRandomizer;
import Fill.CarRandomizer;
import Fill.ItemRandomizer;
import Fill.RootVegetableRandomizer;
    ...
    public void execute(){
        ...
        while (step){
            ...
            switch (choice) {
                case "1":
                    do{
                    Storage<Car> storage= Storage.<Car>getInstance(TempClass.datatype.CAR);
                ...
                case "2":
                    ...
                    do{
                    //Class.Book
                    Storage<Book> storage= Storage.<Book>getInstance(TempClass.datatype.BOOK);
                    int start_index=findFirst(storage,size);
                    Object[] objects = storage.getObjects();
                ...
                case "3":
                    ...
                    do{
                    Storage<RootVegetable> storage= Storage.<RootVegetable>getInstance(TempClass.datatype.ROOTVEGETABLE);
                    int start_index=findFirst(storage,size);
                    Object[] objects = storage.getObjects();
